...
| [coins](coins.md) | earning model owned by the shared `coins.js`; the "subtle, never a gate" usage philosophy + the no-data-loss durability mandate stated in spec | [coins `## Verify`](coins.md#verify-is-it-being-followed) — counter from shared chrome; nothing gated on coins; project coin moments subtle/optional/engagement-tied; **no coins/reader data loss** by hub or node; local store disclosed |
