...
and no automation that reaches across repositories. Each side reads a shallow,
read-only clone of the other when asked to. Both directions track the `dev`
branch.
...
- **Hub reads projects.** The hub keeps read-only shallow clones of each project
  under `assets/references/` (git-ignored, never committed) so their changes can be
  summarised on the blog.
- **Projects read the hub.** Each project keeps its own read-only shallow clone of
...
opposite of the goal here. A throwaway shallow clone in a git-ignored folder
